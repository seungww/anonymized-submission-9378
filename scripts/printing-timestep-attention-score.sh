...
KFOLD=1
#PKTCOUNT="10 20 30 50 70 100 200 300 500 700 1000 2000 3000 5000"
PKTCOUNT="1000"
INPUT_DIM=512
HIDDEN_SIZE=256
NUM_LAYERS=3
...
for value in $PKTCOUNT; do
    TS=$(date +%s)
    DEBUG_PATH="${OUTPUT_DIR}/${STEP}_${DATA_PREFIX}_${TS}.debug"
    LEADERBOARD_PATH="${OUTPUT_DIR}/${STEP}_${DATA_PREFIX}_${TS}.csv"

    # Build full command
    CMD="python $SCRIPT --$STEP --debug_path $DEBUG_PATH --leaderboard_path $LEADERBOARD_PATH --kfold $KFOLD --pktcount $value --input_dim $INPUT_DIM --hidden_size $HIDDEN_SIZE --num_layers $NUM_LAYERS --dropout $DROPOUT --fusion_dim $FUSION_DIM --fc_hidden_size $FC_HIDDEN_SIZE --epoch $EPOCH --lr $LR --feature_attention --timestep_attention --strict"
