...
echo "=== Openworld Evaluation. Script Execution Started at $(date) ===" | tee "$LOG_FILE"
...
for value in $NUM_LAYERS; do
    ...
    LEADERBOARD_PATH="${OUTPUT_DIR}/${STEP}_${DATA_PREFIX}_${TS}.csv"
    # Build full command
    CMD="python $SCRIPT --$STEP --debug_path $DEBUG_PATH --leaderboard_path $LEADERBOARD_PATH --pktcount $PKTCOUNT --input_dim $INPUT_DIM --hidden_size $HIDDEN_SIZE --num_layers $value --dropout $DROPOUT --fusion_dim $FUSION_DIM --fc_hidden_size $FC_HIDDEN_SIZE --epoch $EPOCH --lr $LR --kfold $KFOLD --strict"
    CMD="$CMD --path"
...
echo "=== Openworld Evaluation. Script Execution Finished at $(date) ===" | tee -a "$LOG_FILE"
