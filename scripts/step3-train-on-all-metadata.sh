...
# Metadata list
metadata_names=(
    "pkt_dir" "pkt_ts_rel" "pkt_ts_diff"
...
DEBUG_PATH="${OUTPUT_DIR}/${STEP}_${DATA_PREFIX}.debug"
LEADERBOARD_PATH="${OUTPUT_DIR}/${STEP}_${DATA_PREFIX}.csv"

echo "=== Step3. Script Execution Started at $(date) ===" | tee "$LOG_FILE"
...
# Build full command
CMD="python $SCRIPT --$STEP --debug_path $DEBUG_PATH --leaderboard_path $LEADERBOARD_PATH --pktcount $PKTCOUNT --epoch $EPOCH --batch_size $BATCH_SIZE --input_dim $INPUT_DIM --hidden_size $HIDDEN_SIZE --fusion_dim $FUSION_DIM --feature_attention --strict"
...
echo "=== Step3. Script Execution Finished at $(date) ===" | tee -a "$LOG_FILE"
