...
STEP="train"
SCRIPT="vrscanner.py"
OUTPUT_DIR="output/${BASE_DIR}/varying_pktcount_evaluation"
TS=$(date +%s)
...
for value in $PKTCOUNT; do
    ...
    # Build full command
    CMD="python $SCRIPT --$STEP --debug_path $DEBUG_PATH --leaderboard_path $LEADERBOARD_PATH --kfold $KFOLD --pktcount $value --input_dim $INPUT_DIM --hidden_size $HIDDEN_SIZE --num_layers $NUM_LAYERS --dropout $DROPOUT --fusion_dim $FUSION_DIM --fc_hidden_size $FC_HIDDEN_SIZE --epoch $EPOCH --lr $LR --strict"
